import io,json,re,time,base64,requests
...
JOB=[("huashan_1",["Mount Hua plank walk","Huashan Shaanxi peak"]),
     ("huangshan_1",["Huangshan sea of clouds","Mount Huangshan pine"]),
     ("yungang_1",["Yungang Grottoes","Yungang Grottoes Datong Buddha"])]
def srch(t,n=8):
...
res=json.load(open('imgs.json'))
res.pop('pingyao_1',None)          # era una foto de un tren, no de Pingyao
...
tw,th=330,215; s=Image.new('RGB',(tw*max(len(new),1),th+20),'white'); dd=ImageDraw.Draw(s)
for i,k in enumerate(new):
    s.paste(Image.open(io.BytesIO(base64.b64decode(res[k]['b64']))).resize((tw,th),Image.LANCZOS),(i*tw,0))
    dd.text((i*tw+4,th+4),k,fill='black')
s.save('contactos8.png')
